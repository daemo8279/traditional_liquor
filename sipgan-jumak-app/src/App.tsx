...
function CatalogScreen({ onBack }: { onBack: () => void }) {
  ...
  const [situs, setSitus] = useState<SituLabel[]>([]);
  const [banjus, setBanjus] = useState<BanjuLabel[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    return liquors.filter((l) => {
      ...
      if (situs.length && !situs.some((s) => l.situCategory.includes(s))) return false;
      if (banjus.length && !banjus.some((b) => l.banjuCategory.includes(b))) return false;
      return true;
    });
  }, [query, cats, situs, banjus]);
  ...
  return (
    <section className="catalog-screen">
      ...
      <FilterGroup title="어울리는 자리" options={SITU_OPTIONS} active={situs} onToggle={(v) => toggle(situs, setSitus, v)} />
      <FilterGroup title="어울리는 안주" options={BANJU_OPTIONS.filter((b) => b !== '없음')} active={banjus} onToggle={(v) => toggle(banjus, setBanjus, v)} />
      ...
      <div className="catalog-grid">
        ...
        {filtered.length === 0 && <p className="hint">조건에 맞는 술이 없어요. 필터를 조정해보세요.</p>}
      </div>
