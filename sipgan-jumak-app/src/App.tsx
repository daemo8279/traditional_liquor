...
// v0.9: "인기 태그 24개"만 보여주면 "청량" 같은 실존하는 태그도 유저가 모르고 지나쳐
// 매번 같은 몇몇 태그(→같은 몇몇 술)만 반복 노출되는 문제가 있었다. 그래서 태그를
// 원본 필드별(향/맛/안주/상황/재료)로 묶어, 태그 전체를 필드별로 펼쳐 볼 수 있게 한다.
const TAG_FIELDS: { key: keyof Liquor; label: string }[] = [
  { key: 'aromaTag', label: '향' },
  { key: 'tasteNote', label: '맛' },
  { key: 'banjuTag', label: '안주' },
  { key: 'situTag', label: '상황' },
  { key: 'mainIngredient', label: '재료' },
];
...
// 필드별 태그 빈도 — 태그 브라우저를 향/맛/안주/상황/재료 카테고리로 나눠 보여주기 위함.
const TAG_COUNT_BY_FIELD: Record<string, Map<string, number>> = {};
for (const { key } of TAG_FIELDS) TAG_COUNT_BY_FIELD[key] = new Map();
for (const l of liquors) {
  const tagSet = new Set<string>();
  for (const { key } of TAG_FIELDS) {
    for (const t of splitTags(l[key])) {
      tagSet.add(t);
      const m = TAG_COUNT_BY_FIELD[key];
      m.set(t, (m.get(t) ?? 0) + 1);
    }
  }
  const tags = [...tagSet];
...
// 카테고리별로 빈도순 정렬된 태그 목록 — 술창고 태그 브라우저에서 "이런 태그가 있구나"를
// 한눈에 다 볼 수 있게, 인기 순위로 잘라내지 않고 필드별 전체를 넘긴다.
const TAGS_BY_FIELD: { key: string; label: string; tags: string[] }[] = TAG_FIELDS.map(({ key, label }) => ({
  key,
  label,
  tags: [...TAG_COUNT_BY_FIELD[key].entries()].sort((a, b) => b[1] - a[1]).map(([t]) => t),
}));
...
// 생년월일 입력 범위: 미래 날짜나 비상식적으로 오래된 날짜를 실수로 입력해도
// 아무 경고 없이 계산되던 것을 막기 위해, 입력 가능한 날짜를 오늘~1900-01-01로 제한한다.
function todayISO() {
  const d = new Date();
...
function ChoiceCard<T,>({
  groupLabel, idx, total, question, options, selected, onSelect, onBack, onNext,
}: {
  groupLabel: string;
  idx: number;
  total: number;
  ...
  return (
    <section className="card step-card question-card">
        ...
        {options.map((opt, i) => {
          ...
          return (
            <button
              ...
              onClick={() => { onSelect(opt.value); onNext(); }}
            >
      ...
      {/* 단일 선택 질문(취향 T1~T4, 상황 S1·S3·S4)은 답을 고르는 즉시 다음으로 넘어간다.
          복수 선택인 안주(S2) 화면만 언제 다 골랐는지 알 수 없어서 [다음] 버튼이 따로 있다 —
          그래서 여기 단일 선택 화면들은 [이전]만 두고, 고르면 자동으로 넘어가도록 한다. */}
...
function FilterGroup<T extends string>({
  title, options, active, onToggle, onClear,
}: { title: string; options: T[]; active: T[]; onToggle: (v: T) => void; onClear?: () => void }) {
  return (
    <div className="filter-group">
      ...
      <div className="filter-chip-row">
        {onClear && (
          <button
            className={`filter-chip ${active.length === 0 ? 'active' : ''}`}
            onClick={onClear}
          >
            전체
          </button>
...
// 카테고리(향/맛/안주/상황/재료) 하나의 태그 목록. 기본으로는 앞쪽 일부만 보여주고,
// "태그 더보기"를 누르면 그 카테고리의 전체 태그가 다 펼쳐진다 — 유저가 어떤 태그가
// 있는지 몰라서 매번 같은 몇 개만 고르게 되는 문제를 막기 위함.
const TAG_PREVIEW_COUNT = 10;

function TagCategory({
  label, tags, activeTags, onAdd,
}: { label: string; tags: string[]; activeTags: string[]; onAdd: (t: string) => void }) {
  const [expanded, setExpanded] = useState(false);
  const visibleTags = tags.filter((t) => !activeTags.includes(t));
  const shown = expanded ? visibleTags : visibleTags.slice(0, TAG_PREVIEW_COUNT);
  const hiddenCount = visibleTags.length - shown.length;
  return (
    <div className="tag-category">
      <div className="tag-category-label">{label} <span className="tag-category-count">{tags.length}개</span></div>
      <div className="filter-chip-row">
        {shown.map((t) => (
        ...
        {hiddenCount > 0 && (
          <button className="filter-chip filter-chip-more" onClick={() => setExpanded(true)}>
            +{hiddenCount}개 더보기
          </button>
        )}
        {expanded && visibleTags.length > TAG_PREVIEW_COUNT && (
          <button className="filter-chip filter-chip-more" onClick={() => setExpanded(false)}>
            접기
          </button>
        )}
      </div>
...
function TagSearch({
  ...
  return (
    <div className="filter-group">
      <div className="filter-title">태그로 찾기 (총 {ALL_TAGS.length}개 태그 · 향·맛·안주·상황·재료 데이터 기반)</div>
      <input
        type="text"
        placeholder="태그를 검색하거나, 아래 카테고리에서 골라보세요"
        value={tagQuery}
      ...
      {tagQuery ? (
        <div className="filter-chip-row">
          {suggestions.length > 0 ? (
            ...
            <span className="hint">일치하는 태그가 없어요.</span>
          )}
        </div>
      ) : (
        <div className="tag-category-list">
          {TAGS_BY_FIELD.map(({ key, label, tags }) => (
            <TagCategory key={key} label={label} tags={tags} activeTags={activeTags} onAdd={onAdd} />
          ))}
        </div>
      )}
    </div>
  ...
  const filtered = useMemo(() => {
    ...
    return liquors.filter((l) => {
      ...
      if (activeTags.length) {
        // 태그를 여러 개 고르면 전부 만족(AND)하는 술만 남겨서, 태그 몇 개만 겹쳐도
        // 후보가 순식간에 0이 돼버렸다. "이 중 하나라도 있으면" 보여주는 OR로 바꾼다.
...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    // 취향 신호가 약할 때(예: 취향 질문에 '중간'만 고른 경우) 사주 추천(B)과 취향 추천(C)의
    // 가중치가 거의 같아져, 두 카드가 같은 술을 추천하는 경우가 시뮬레이션상 40~50%나 됐다.
    // 그래서 C는 B가 이미 고른 술을 후보 풀에서 제외하고 다시 계산해, 두 카드가 항상
    // 서로 다른 술을 보여주도록 한다(정합도 계산 자체는 전체 120종 통계를 그대로 쓴다).
    const bTopId = b.top3[0]?.liquor.id;
    const cPool = bTopId ? liquors.filter((l) => l.id !== bTopId) : liquors;
    const c = recommend(weightC, cPool, stats, effSitu.S2, today, 1);
    return { b, c };
  ...
  function submitBirth() {
    ...
    // <input type="date">의 min/max는 대부분 브라우저에서 지켜지지만, 일부 환경(수동
    // 입력, 구형 모바일 브라우저)에서는 우회될 수 있어 제출 시점에 한 번 더 검증한다.
    if (birth > todayISO()) { setBirthError('아직 오지 않은 날짜예요. 다시 확인해주세요.'); return; }
    if (birth < MIN_BIRTH_DATE) { setBirthError('너무 오래된 날짜예요. 다시 확인해주세요.'); return; }
    try {
      calcDayPillar(birth);
      setBirthError('');
      goNext();
  ...
  return (
    <div className="shell">
      ...
      {step === 'S2' && (
        <section className="card step-card question-card">
          <h2>상황 (2/4)</h2>
          <div className="qtext qtext-solo">함께 먹을 안주가 있다면?</div>
          <div className="multi-badge">
            <span className="multi-badge-icon">☑</span>여러 개 골라도 좋아요 — 다 고른 뒤 [다음]을 눌러주세요
          </div>
          <div className="opt-col opt-col-multi">
            {BANJU_OPTIONS.map((opt) => {
              const active = situ.S2.includes(opt);
              return (
                <button
                  key={opt}
                  className={`opt-btn-v opt-btn-v-multi ${active ? 'active' : ''}`}
                  onClick={() => toggleBanju(opt)}
                >
                  <span className="opt-checkbox">{active ? '☑' : '☐'}</span>
