...
type Step = 'birth' | 'T1' | 'T2' | 'T3' | 'T4' | 'S1' | 'S2' | 'S3' | 'S4' | 'result';
const STEP_ORDER: Step[] = ['birth', 'T1', 'T2', 'T3', 'T4', 'S1', 'S2', 'S3', 'S4', 'result'];
  ...
  return (
    <div className="progress-dots">
      {Array.from({ length: total }, (_, i) => (
        <span key={i} className={`dot ${i === current ? 'active' : i < current ? 'done' : ''}`} />
      ))}
    </div>
  );
}
...
function ChoiceCard<T,>({
  ...
  return (
    <section className="card step-card question-card">
      ...
      <div className="opt-row opt-row-solo">
        {options.map((opt, i) => (
          <button
            key={i}
            className={`opt-btn ${selected === opt.value ? 'active' : ''}`}
...
export default function App() {
  ...
  return (
    <div className="shell">
      ...
      {step !== 'result' && (
        <ProgressDots current={stepIndex(step)} total={STEP_ORDER.length - 1} />
      )}
        ...
        (key, i) =>
          step === key && (
            <ChoiceCard
              ...
              options={TASTE_LABELS[key].opts.map((label, oi) => ({ label, value: oi + 1 }))}
              selected={taste[key]}
      ...
      {step === 'S1' && (
        <ChoiceCard
          ...
          options={SITU_OPTIONS.map((opt) => ({ label: opt, value: opt }))}
          selected={situ.S1}
      ...
      {step === 'S3' && (
        <ChoiceCard
          ...
          options={(['가볍게', '보통', '진하게'] as const).map((opt) => ({ label: opt, value: opt }))}
          selected={situ.S3}
      ...
      {step === 'S4' && (
        <ChoiceCard
          ...
          options={['아주 익숙한 것', '무난한 것', '중간', '개성 있는 것', '완전 새로운 것'].map((label, i) => ({ label, value: i + 1 }))}
          selected={situ.S4}
      ...
      {step === 'result' && profile && results && (
        <section className="results">
            ...
            <div className="profile-grid">
              <div className="profile-text">
                ...
                <div className="profile-head">
                  <span className="profile-gan">{profile.label}일주</span>
                  <span className="profile-hanja">{profile.hanjaLabel}</span>
                  <span className="profile-elem">{profile.gan.element} · {profile.gan.yinyang}</span>
                </div>
          ...
          <div className="two-col">
            <div>
              <div className="block-title accent-a">사주(일주)가 추천하는 오늘의 전통주</div>
