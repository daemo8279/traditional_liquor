...
import type { Axis, DerivedAxis, Liquor, TenganProfile, TasteAnswers, SituAnswers, SituLabel } from './types';
...
// ---- 십천간 가중치 (= profile값 - 3) ----
export function tenganWeights(profile: TenganProfile): Weights {
  const w: Weights = {};
  for (const ax of DERIVED_AXES) w[ax] = profile.axes5[ax] - 3;
  return w;
}
...
function accessBonus(liquor: Liquor): number {
  if (liquor.accessibility === 'A') return 10;
  if (liquor.accessibility === 'C') return -5;
  return 0;
}
 ...
 * final_score = minmax_100(alignment) + 안주 가산점(0~30) + 구매접근성 보정(A:+10/C:-5), clip 0~100
 */
